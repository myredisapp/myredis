...
#
#   1. 必需环境变量齐全且非空。
#   2. 更新包签名私钥已配置（自动更新依赖它，缺了会静默失效）。
#   3. SSH 认证方式有且只有一种（私钥或密码）。
#   4. SSH 端口（若提供）是合法数字。
#   5. 目标主机可达且凭据可登录。
#   6. scp 试传一个临时文件并确认落地，随后清理。
#
...
if [[ -z "${TAURI_SIGNING_PRIVATE_KEY:-}" && -z "${TAURI_SIGNING_PRIVATE_KEY_PATH:-}" ]]; then
  ...
  echo "::error::该私钥用于给自动更新包签名，丢失后已发布的应用将无法再收到更新，请妥善备份。" >&2
  echo "::error::生成密钥对：tauri signer generate -w ~/.tauri/myredis-updater.key" >&2
  exit 1
fi

# ---------------------------------------------------------------------------
# 第 3 步 — SSH 认证方式有且只有一种。
# ---------------------------------------------------------------------------
AUTH_METHODS=0
