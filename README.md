...
- **全类型 Key 编辑**：String / Hash / List / Set / ZSet / Stream 内容加载与字段级编辑，TTL 修改，批量删除，重命名 / 复制（`RENAME` / `COPY`）
- **实时命令监控**：底部面板实时滚动展示服务器上执行的命令（`MONITOR`），支持过滤、自动滚动与暂停
- **大数据量友好**：`SCAN` 游标分页 + 虚拟滚动，几十万 Key 不卡；搜索下推后端 `SCAN MATCH`
- **内置终端**：真实命令转发，redis-cli 风格输出，只读连接自动拦截写命令（与实时监控共用底部面板的双标签）
- **TLS 加密连接**：`rediss://`，支持自签名证书跳过校验；密码存系统密钥链不落盘
